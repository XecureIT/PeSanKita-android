...
public class GroupMessageProcessor {
                                                  ...
                                                  boolean outgoing)
  {
    ...
    List<Address>           members = group.getMembers().isPresent() ? new LinkedList<Address>() : Collections.<Address>emptyList();
    ...
    Address                 owner   = group.getOwner().isPresent() ? Address.fromSerialized(group.getOwner().get()) : null;
    List<Address>           admins  = group.getAdmins().isPresent() ? new LinkedList<Address>() : Collections.<Address>emptyList();
    ...
    database.create(id, group.getName().orNull(), members, owner, admins,
...
//    addedMembers.removeAll(recordMembers);
    ...
    database.updateMembers(id, Address.fromExternalList(context, group.getMembers().or(Collections.<String>emptyList())));
...
//    }
    ...
    if(group.getOwner().isPresent()) {
      ...
      database.updateAdmins(id, Address.fromExternalList(context, messageListAdmins));
    ...
    if (missingMembers.size() > 0) {
      for (Address missingMember : missingMembers) {
        if(missingMember.serialize().equals(TextSecurePreferences.getLocalNumber(context))) {
  ...
  private static GroupContext.Builder createGroupContext(SignalServiceGroup group) {
    ...
    if (group.getOwner().isPresent()) {
      builder.setOwner(group.getOwner().get());
    }
